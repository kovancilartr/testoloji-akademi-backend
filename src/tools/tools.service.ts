  ...
  async magicScan(
    ...
    if (!scriptPath) {
      ...
      throw new InternalServerErrorException(
        'Görüntü işleme betiği bulunamadı.',
      );
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      pythonProcess.on('error', (err: any) => {
        if (err.code === 'ENOENT' && pythonCommand === 'python3') {
          ...
          reject(
            new InternalServerErrorException('Sunucuda Python başlatılamadı.'),
          );
        }
      });
      ...
      const setupHandlers = (proc: any) => {
        ...
        proc.on('close', (code: any) => {
          ...
          if (code !== 0) {
            ...
            return reject(
              new InternalServerErrorException('Image processing failed'),
            );
          }
