...
export class QuestionsService {
    ...
    async create(projectId: string, imageUrl: string, userRole: Role, userTier: SubscriptionTier, userId: string, width: number, height: number) {
        return (await this.bulkCreate(projectId, [{ imageUrl, width, height }], userRole, userTier, userId))[0];
    }

    async bulkCreate(
        projectId: string,
        questions: { imageUrl: string, width: number, height: number, difficulty?: number | null, correctAnswer?: string | null }[],
        userRole: Role,
