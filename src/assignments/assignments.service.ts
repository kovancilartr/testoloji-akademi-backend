...
export class AssignmentsService {
  ...
  async deleteAssignment(teacherId: string, assignmentId: string) {
    ...
    if (studentUserId) {
      try {
        ...
        console.warn(
          `Could not send deletion notification to student ${studentUserId}`,
          error,
        );
      }
    }
  ...
  async undoSubmitAssignment(userId: string, assignmentId: string) {
    ...
    if (assignment.type === 'TEST')
      throw new ForbiddenException('Sınav sonuçları geri alınamaz.');
  ...
  async createMakeUpAssignment(userId: string, originalAssignmentId: string) {
    ...
    if (!originalAssignment || !originalAssignment.project) {
      throw new NotFoundException(
        'Orijinal sınav bulunamadı veya henüz tamamlanmamış.',
      );
    }

    const answers = (originalAssignment.answers || {}) as Record<
      string,
      string
    >;
    ...
    if (questionsToReview.length === 0) {
      throw new ForbiddenException(
        'Bu sınavda tekrar edilecek hatalı veya boş soru bulunmuyor.',
      );
    }

    // Testin orjinal ismini bul (eğer bu testin kendisi zaten bir Tekrar Testi ise onu ayıkla)
    const baseTitleMatch = originalAssignment.title.match(
      /(?:\(\d+\)\s*)?Tekrar Testi:\s*(.*)/,
    );
    const baseTitle = baseTitleMatch
      ? baseTitleMatch[1]
      : originalAssignment.title;
    ...
    const prefix =
      totalMakeUpsCount > 0
        ? `(${totalMakeUpsCount + 1}) Tekrar Testi:`
        : `Tekrar Testi:`;
  ...
  async createCombinedMakeUpAssignment(userId: string) {
    ...
    if (pastAssignments.length === 0) {
      throw new ForbiddenException(
        'Telafi edilecek hatalı testiniz bulunmuyor.',
      );
    }
    ...
    if (allQuestionsToReview.length === 0) {
      throw new ForbiddenException(
        'Geçmiş testlerde kaydedilmiş hiçbir hatalı veya boş soru bulunamadı.',
      );
    }
