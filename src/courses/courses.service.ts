...
export class CoursesService {
  ...
  async updateCourse(
    courseId: string,
    data: Partial<CreateCourseDto> & { isPublished?: boolean },
  ) {
  ...
  async deleteModule(moduleId: string) {
    // Modüldeki tüm içerikleri önce cascade ile sil
    const contents = await this.prisma.courseContent.findMany({
      where: { moduleId },
      select: { id: true },
    });

    // Her içeriği cascade mantığı ile sil (assignment, progress, project dahil)
  ...
  async deleteContent(contentId: string) {
    // İçeriği bağlantıları ile birlikte getir
    ...
    // Transaction ile öğrenciye ait tüm ilişkili kayıtları sil
    return await this.prisma.$transaction(async (tx) => {
      // 1. İçerik ilerlemelerini sil
      await tx.contentProgress.deleteMany({
        where: { contentId },
      });

      // 2. contentId ile EŞLEŞen assignment'ları sil.
      //    Ayrıca aynı projectId ile eşleşip contentId = null olan eski kayıtları da sil.
      ...
      // 3. İçeriği sil
      //    NOT: Bağlı Project (TestMaker projesi) silinmez — öğretmenin kendi projesidir.
      await tx.courseContent.delete({
        where: { id: contentId },
      });

      return { success: true, deletedContentId: contentId };
    });
  }
      ...
      course.modules.forEach((module) => {
        module.contents.forEach((content: any) => {
          if (content.type === 'TEST' && content.projectId) {
            let contentAssignments = allAssignments.filter(
              (a) => a.contentId === content.id,
            );

            if (contentAssignments.length === 0) {
              contentAssignments = allAssignments.filter(
                (a) =>
