...
export class StudentQuestionsService {
  ...
  async create(userId: string, createQuestionDto: CreateQuestionDto) {
      ...
      include: {
        ...
        questions: {
          where: {
            ...
            OR: [{ isDeleted: false }, { status: 'ANSWERED' }],
          },
        },
      },
    });
    ...
    const tierDefault =
      tierQuestionLimits[student.user?.tier as SubscriptionTier] || 1;
    const DAILY_LIMIT =
      student.dailyQuestionLimit > 0 ? student.dailyQuestionLimit : tierDefault;

    const todayCount = student.questions.length;

    if (todayCount >= DAILY_LIMIT) {
      throw new BadRequestException(
        `Günlük soru limitiniz (${DAILY_LIMIT}) dolmuştur. Paketine göre limitin dolmuştur, yarın tekrar sorabilirsin.`,
      );
    }
    ...
    if (createQuestionDto.imageUrl.startsWith('data:image')) {
      try {
        const uploadRes = await this.uploadthing.uploadBase64(
          createQuestionDto.imageUrl,
          `question_${userId}_${Date.now()}.png`,
        );
  ...
  async answer(
    ...
    if (answerDto.answerUrl && answerDto.answerUrl.startsWith('data:image')) {
      try {
        const uploadRes = await this.uploadthing.uploadBase64(
          answerDto.answerUrl,
          `answer_${teacherId}_${Date.now()}.png`,
        );
        if (uploadRes && uploadRes.url) {
        ...
        throw new BadRequestException(
          'Cevap görseli yüklenirken bir hata oluştu.',
        );
      }
    }
