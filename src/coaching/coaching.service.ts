...
export class CoachingService {
  ...
  private async ensureGenAI() {
    const dbApiKey =
      await this.systemSettingsService.getSetting('GEMINI_API_KEY');
    const apiKey = dbApiKey || this.configService.get<string>('GEMINI_API_KEY');

    if (!apiKey) {
      throw new InternalServerErrorException(
        'Yapay zeka servisi API anahtarı yapılandırılmamış.',
      );
    }
  ...
  async askAi(userId: string, dto: AskAiDto) {
    ...
    if (usage.count >= usage.limit) {
      throw new ForbiddenException(
        `Günlük yapay zeka kullanım limitinize ulaştınız (${usage.limit}/${usage.limit}). Yarın tekrar deneyebilirsiniz.`,
      );
    }
    ...
    return {
      ...
      jobId: job.id,
      message:
        'Yapay zeka analizi sıraya alındı. Kısa süre içinde yanıtlanacak.',
    };
  }
  ...
  async processAskAiInternal(userId: string, dto: AskAiDto) {
    ...
    if (usage.count >= usage.limit) {
      throw new ForbiddenException(
        `Günlük yapay zeka kullanım limitine ulaşıldı.`,
      );
    }
    ...
    try {
      const modelConfig =
        (await this.systemSettingsService.getSetting('GEMINI_MODEL')) ||
        'gemini-2.5-flash';
   ...
   */
  async analyzeStudentForTeacher(
    teacherUserId: string,
    studentId: string,
    dto: AnalyzeProgressDto,
  ) {
    // Güvenlik kontrolü
    ...
    const isGeneralReport =
      dto.query.includes('Performans Değerlendirmesi') ||
      dto.query.includes('Gelişim Raporu');
    if (isGeneralReport) {
      const today = new Date();
      today.setHours(0, 0, 0, 0);
    ...
    if (usage.count >= usage.limit) {
      throw new ForbiddenException(
        `Günlük yapay zeka kullanım limitinize ulaştınız. Yarın tekrar deneyebilirsiniz.`,
      );
    }
   ...
   * Kuyruk üzerinden işlemci tarafından çağrılır.
   */
  async processAnalyzeProgressInternal(
    userId: string,
    dto: AnalyzeProgressDto,
  ) {
    const usage = await this.getUsage(userId);
    ...
    console.log(
      `Query type: ${isExamAnalysis ? 'SINAV ANALİZİ' : 'GENEL KOÇLUK'}`,
    );
    ...
    try {
      ...
      if (!studentData) {
        try {
          let targetStudent;
          if (dto.studentId) {
            targetStudent = await this.prisma.student.findUnique({
          ...
          if (targetStudent) {
            studentData = await this.analyticsService.getStudentOverview(
              userId,
              Role.STUDENT,
              targetStudent.id,
            );
            ...
            const scheduleSummary =
              await this.schedulesService.getScheduleSummary(targetStudent.id);
          ...
          console.warn(
            'Analytics/Schedule data could not be fetched:',
            userId,
            dto.studentId,
          );
        }
      ...
      if (isExamAnalysis) {
        ...
        if (latestAssignment) {
          ...
          if ((latestAssignment as any).aiAnalysis) {
            console.log(
              `Cached analysis found for assignment: ${assignmentId}`,
            );
        ...
        const promptText = this.buildAnalysisPrompt(
          { ...dto, studentData },
          reversedHistory,
          true,
        );
        ...
        if (latestAssignment && latestAssignment.project) {
          ...
          if (pdfBase64) {
            ...
            parts.push({
              text: '[PDF oluşturulamadı, lütfen metin bazlı analize devam et.]\n',
            });
          }
        } else {
          parts.push({
            text: '[Tamamlanmış sınav bulunamadı. Genel öneriler ver.]\n',
          });
        }
      } else {
        // ===== GENEL KOÇLUK MODU =====
      ...
      try {
        ...
        console.warn(
          `Primary model ${modelConfig} failed. Trying fallback... Error: ${genError}`,
        );
      ...
      const footerBadge =
        "Bu analiz 'Akademi Kovancılar' için özel olarak hazırlanmış";
      ...
      await this.logAiUsage(
        finalModelUsed,
        response.usageMetadata,
        isExamAnalysis ? 'AnalyzeExam' : 'CoachChat',
        userId,
      );
      ...
      if (
        dto.query.includes('Kişisel Gelişim Raporu') ||
        dto.query.includes('Performans Değerlendirmesi') ||
        dto.query.includes('Gelişim Raporu')
      ) {
      ...
      if (dto.studentId) {
        const targetStudent = await this.prisma.student.findUnique({
          where: { id: dto.studentId },
        });
  ...
  private isExamAnalysisQuery(query: string): boolean {
    ...
    const lowerQuery = query
      ...
      .replace(/ç/g, 'c')
      .replace(/ğ/g, 'g');
    const normalizedKeywords = examKeywords.map((k) =>
      k
        .toLowerCase()
        .replace(/ı/g, 'i')
        .replace(/ö/g, 'o')
   ...
   */
  private async generateExamAnalysisPdf(
    examTitle: string,
    questions: any[],
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        doc
          .fontSize(14)
          .text('Sınav Analiz Raporu ve Sorular', { align: 'center' });
        doc.moveDown();
        ...
        for (const [index, question] of questions.entries()) {
          ...
          doc
            .fontSize(12)
            .fillColor('black')
            .text(`Soru ${index + 1}:`, { underline: true });
          doc.moveDown(0.5);
          ...
          doc
            .fillColor('black')
            .text(`Öğrenci Cevabı: ${studentAnswer || 'BOŞ'}`);
          doc.text(`Doğru Cevap: ${question.correctAnswer}`);
          doc.moveDown();

          if (question.imageUrl) {
            try {
              const imgBuffer = await this.fetchImageAsBuffer(
                question.imageUrl,
              );
              if (imgBuffer) {
                doc.image(imgBuffer, {
        ...
        doc
          .fontSize(16)
          .fillColor('black')
          .text('Cevap Anahtarı', { align: 'center', underline: true });
        doc.moveDown();
        ...
        questions.forEach((q, i) => {
          ...
          doc
            .fontSize(10)
            .text(
              `${i + 1}. Doğru: ${q.correctAnswer} (Sen: ${studentAnswer || '-'}) ${symbol}`,
            );
        });
   ...
   */
  async getStudentHistoryForTeacher(
    teacherUserId: string,
    studentId: string,
    page: number = 1,
    limit: number = 5,
    ...
    const student = await this.prisma.student.findUnique({
      where: isAdmin
        ? { id: studentId }
        : { id: studentId, teacherId: teacherUserId },
    });

    if (!student) {
      throw new ForbiddenException(
        'Bu öğrencinin verilerine erişim yetkiniz yok veya öğrenci bulunamadı.',
      );
    }
  ...
  private buildAnalysisPrompt(
    dto: AnalyzeProgressDto,
    history: any[] = [],
    includeExamInstructions: boolean = true,
  ): string {
    ...
    if (studentData) {
      ...
      if (
        studentData.recentFocusSessions &&
        studentData.recentFocusSessions.length > 0
      ) {
        const totalFocusMinutes = studentData.recentFocusSessions.reduce(
          (acc: number, curr: any) => acc + curr.duration,
          0,
        );
        const mostStudiedSubjects = studentData.recentFocusSessions.reduce(
          (acc: any, curr: any) => {
