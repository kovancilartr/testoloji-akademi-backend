...
export class AnalyticsService {
    ...
    async getStudentOverview(reqUserId: string, role: Role, studentId: string) {
                ...
                assignments: {
                    ...
                    include: {
                        project: {
                            select: {
                                questions: {
                                    select: { id: true, imageUrl: true, correctAnswer: true, order: true }
                                }
                            }
                        }
                    }
        ...
        const courseProgress = student.enrollments.map(enrollment => {
            ...
            const totalContents = course.modules.reduce((acc, mod) => acc + mod.contents.length, 0);
            const completedContents = student.contentProgress.filter(cp =>
                cp.status === 'COMPLETED' &&
                course.modules.some(mod => mod.id === cp.contentId || mod.contents.some(c => c.id === cp.contentId))
            ).length;
        ...
        const avgScore = exams.length > 0
            ? Math.round(exams.reduce((acc, ex) => acc + (ex.grade || 0), 0) / exams.length)
            : 0;

        const scoreHistory = exams.map(ex => {
        ...
        const suggestions = this.generateSuggestions(avgScore, courseProgress, exams);
    ...
    private generateSuggestions(avgScore: number, courseProgress: any[], exams: any[]) {
        const suggestions: { type: string; message: string; target: string; }[] = [];

        if (avgScore < 50 && exams.length > 0) {
            suggestions.push({
                type: 'WARNING',
                message: "Sınav ortalaman düşük görünüyor. Temel konuları tekrar etmeni öneririm.",
                target: "Tüm Konular"
            });
        }
        ...
        if (avgScore > 85) {
            suggestions.push({
                type: 'SUCCESS',
                message: "Harika gidiyorsun! Sınav sonuçların çok başarılı. Yeni ve daha zorlu konulara geçebilirsin.",
        ...
        if (suggestions.length === 0) {
            suggestions.push({
                type: 'INFO',
                message: "Çalışmalarına devam et, ilerlemeni buradan takip edebilirsin.",
                target: "Genel"
            });
        }
    ...
    async getTeacherOverview(teacherId: string) {
        ...
        return {
            ...
            studentData,
            averageClassGrade: Math.round(studentData.reduce((acc, s) => acc + s.avgGrade, 0) / (studentData.length || 1))
        };
    }
}
